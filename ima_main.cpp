...
int numtreesarray[] = {0,1,1,3,18,180,2700,56700,1587600}; // numer of distinct tree topologies with different #'s of populations// can use an unsigned short 16 bit to go up to 7, but not 8 
// no longer used int pairhasharraysize[] = {0,0,0,51,500,2501,20002,100001,1000001}; // size of array so values generated by pairhash() will all be unique and give no collisions      
...
free_ima_main_stuff ()
{
  ...
  //XFREE (swapper); // noticed not being used 6/7/2017
  //XFREE (swappee); // noticed not being used 6/7/2017
...
//printf("2 %d %llu\n",currentid,remained_starttime);
  ...
  if ((argc == 2 && ((char) toupper (argv[1][0]) == 'H' || (char) toupper (argv[1][1]) == 'H') ) || argc == 1)
  {
    ...
    printf ("==================\n\n");
      ...
      printf ("-z  Number of steps between screen output (default is %d) (do not use with LOAD-GENEALOGY mode -r0)\n",PRINTINTDEFAULT);
      fflush(stdout);
    }
#ifdef MPI_ENABLED
  MPI_Finalize();
  ...
  /* setting  mcmc step intervals:
    ...
    if sampling phylogenies: 
      genealogies are not sampled
        savegenealogyint = DONOTSAVE_BECAUSE_PHYLOGENYESTIMATION -1 
  */
  ...
  {
    savegenealogyint = DONOTSAVE_BECAUSE_PHYLOGENYESTIMATION;   // do not save genealogies
  }
...
begin_outputfile_info_string (void)
{
  ...
  *fpstri = 0;
  ...
  SP "\nJob Started: %s\n",timeinfostring);
  //SP "\n================================\n");
  //SP "INPUT AND STARTING INFORMATION \n");
  //SP "================================\n");
  //char bstr[3*BANNERMAXLENGTH], btextstr[BANNERMAXLENGTH];
  //strcpy(bannertext,"input and starting information");
  //SP "%s",outputbanner(bannertext));
  ...
  if (!runoptions[LOADRUN])
  {
    ...
    {
    case TIMESTEPS:
      SP "  Burnin period, # steps: %li ", burnduration);
      break;
    case TIMEINF:
      SP "  Burnin period, # seconds: %li (total burn duration depends on IMburn file status)", burnduration);
      break;
...
screen and file io are done almost entirely only on the head node */
...
/* updatecounter is a simple stucture to determine when things get updated
  ...
  */ 
  ...
  if (step==0)
  {
    ...
    //migpriornit_uc.interval =   INT_MAX;
    //qpriornit_uc.interval = INT_MAX;
  }
...
checkgenealogyweights(ci);
  ...
  if (npops > 1  && modeloptions[POPTREETOPOLOGYUPDATE] == 1) 
  {
    if (poptopol_uc.count >= poptopol_uc.interval )
    {
      for (ci = 0; ci < numchainspp; ci++)
      {
//printf("step %d chain %d treenum %d uni %.4lf",step,ci,C[ci]->poptreenum,uniform());        
        changed = change_poptree (ci,&trypoptopolchange,&poptopolchange,&trypoptmrcachange,&poptmrcachange, 1);
        #ifdef TURNONCHECKS
        pcheck(ci,3);
        #endif
//printf(" changed %d treenum %d uni %.4lf\n",changed,C[ci]->poptreenum,uniform());
        if (ci==z) // only record acceptance and tries from the cold chain
        {
...
#endif 
        //printf ("%d  %d attempts: %d  within %d between %d\n",step,currentid,swaptries_per_cpu,numattemptwithin,numattemptbetween);
	     }
      else // numprocesses == 1
      {
...
//printf("9 %d %llu\n",currentid,burntrendtime);
  ...
  if (currentid == HEADNODE)
  {
    ...
    //strcpy(bannertext,"printing burn trend file");
    //printf("%s",outputbanner(bannertext));
...
//printf("%d %d %llu  %d\n",step,currentid,(long long unsigned)  timer,tempburndone);
//fflush(stdout);
#endif
      }
      break;
...
/*
 record values from the MCMC
 if multiple cpus,  should only record from the one that hosts the cold chain
 ...
 */
  ...
  if (modeloptions[POPTREETOPOLOGYUPDATE] == 1) // add the current topology number from the cold chain to poptopologysequence
  {
	    if (z >= 0 && currentid == HEADNODE) 
     {
       poptopologysequence.vals[poptopologysequence.currentlength] = (unsigned short) C[z]->poptreenum;
	    ...
	    if (z < 0 && currentid == HEADNODE) // receive roottime 
     {
		     ...
		     if (rc != MPI_SUCCESS) MPI_Abort(MPI_COMM_WORLD, rc);
       assert (ptn >= 0 && ptn < numtreesarray[npops - modeloptions[ADDGHOSTPOP]]);
       poptopologysequence.vals[poptopologysequence.currentlength] = (unsigned short) ptn;
     ...
     if (currentid == HEADNODE)
     {
       ...
       {
         poptopologysequence.vals =  static_cast<unsigned short *>  (realloc (poptopologysequence.vals,(poptopologysequence.maxlength + POPTOPOLOGYSEQUENCELENGTH) * sizeof(unsigned short)));
...
//AS: ideally should do a check for cduration mode here as well and allocate gsampinflocal accordingly
  ...
  else
  {
	...
	int z = whichiscoldchain();
	//AS: Debug only
	//std::cout << "Cold chain here is " << z << " on chain "<< currentid <<"\n";
	if (currentid == HEADNODE) {
		if ( z >= 0) {
    ...
    if (hiddenoptions[HIDDENGENEALOGY] && hiddenoptions[GSAMPINFOEXTRA] == 1)
    {
		...
		//XFREE(gsampinflocal); in wrong place
...
/*   printoutput()  print ascii trends*/  
    ...
    if (!outputoptions[DONTPRINTASCIITREND] && runmode != LOADGmode4)
    {
      ...
      {
        //FP "\n\n===================================================\n");
        //FP "ASCII Plots of Parameter Trends \n");
        //FP "===================================================\n");
        //strcpy(bannertext,"ASCII Plots of Parameter Trends");
...
void intervaloutput (FILE * outto, int currentid)
{
  ...
  {
#ifdef MPI_ENABLED
    rc = MPI_Reduce(&hilike,&hilike_rec,1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (rc != MPI_SUCCESS) MPI_Abort(MPI_COMM_WORLD, rc);
    rc = MPI_Reduce(&hiprob,&hiprob_rec,1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (rc != MPI_SUCCESS) MPI_Abort(MPI_COMM_WORLD, rc);
    rc = MPI_Reduce(&like,&like_rec,1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (rc != MPI_SUCCESS) MPI_Abort(MPI_COMM_WORLD, rc);
    rc = MPI_Reduce(&probg,&probg_rec,1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    if (rc != MPI_SUCCESS) MPI_Abort(MPI_COMM_WORLD, rc);
#endif
  }
  else
  ...
  #endif
  /* hiprob and hilike collection. Note - this code does not also appear in file printing functions
    so hiprob and hilike in output will only be as current as most recent call to intervaloutput().
    and if intervaloutput() has not been called they will get printed as 0's
   */ 
  ...
  if (numprocesses > 1)
  {
	  ...
	  if (rc != MPI_SUCCESS)
		...
		if (rc != MPI_SUCCESS)
				MPI_Abort(MPI_COMM_WORLD, rc);
//printf("currentid %d chain0topolupdates %d  chain0topol_rec %d chain0topolswaps %d chain0topolswaps_rec %d\n",currentid,chain0topolupdates,chain0topol_rec,chain0topolswaps, chain0topolswaps_rec);
   }
#endif	
    ...
    {
      //if (outto != NULL) fprintf(outto,"Current Population Topology #: %d   Current Topology String: %s\n",poptreenum_rec,alltreestrings[poptreenum_rec]); 
...
void check_to_record (int currentid)
{
  ...
  //if (j == savegenealogyint && (hiddenoptions[HIDDENGENEALOGY]==0 || hiddenoptions[GSAMPINFOEXTRA]==1))
